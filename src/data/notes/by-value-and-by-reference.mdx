...
When you assign a variable to a primitive value `var a = "house"` you are giving it an address in memory. Let's try an analogy: imagine my neighbors really liked my house so they decided to make one exactly like mine, they build it but it’s in a different address (a new spot in memory). Some years later they decide they want to change the color of the painting, so they paint yellow, but it obviously won’t affect mine.
